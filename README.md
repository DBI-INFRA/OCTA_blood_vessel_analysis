...
- **`thresholding`**: The following two parameters control the image segmentation step after filtering. Both the chosen method and sensitivity have a significant influence on how the image is separated into vessels and background. See for instance [Peter Bankhead's e-book chapter on thresholding](https://bioimagebook.github.io/chapters/2-processing/3-thresholding/thresholding.html) for an introduction to thresholding and different thresholding methods. 
  - **`.method`**: The thresholding method applied to segment the image into foreground (vessels) and background. The options available are `local_adaptive_thresholding` (the default method, [MATLAB adaptthresh](https://se.mathworks.com/help/images/ref/adaptthresh.html)), `otsu_thresholding` ([MATLAB graythresh](https://se.mathworks.com/help/images/ref/graythresh.html)) and `fuzzy_thresholding` (*helper_scripts/fuzzy_thresholding.m* from the [OCTAVA toolbox](https://github.com/GUntracht/OCTAVA/tree/main))
...
- **"InputParameters.mat"**: .mat file with all new user input variables appended to the existing file, including `median_filter_size`, `frangi_opts`, and `thresholding.method`and `thresholding.sensitivity`.
- **"SPD_not_found.txt"**: Text file that will log all images where the SPD depth could not be calculated. In these cases, you can manually select the SPD depth by examining the graphs in "2_CLD_SPD_estimation/" and editing "ImageSummary.csv" as specified above.
